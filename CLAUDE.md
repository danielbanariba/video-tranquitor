...
2. **Transcribe** — selected by `config.transcriber`:
   ...
   - `ensemble` → `transcribers/ensemble.py`: runs `whisper.cpp` (in a thread) **and** WhisperX (in a `ProcessPoolExecutor` with `spawn` context, to isolate CUDA state) **in parallel**; both outputs are encoded as TOON and arbitrated by the configured LLM. Falls back to whichever transcriber succeeded if one fails, or to WhisperX if arbitration fails.
     **Measured and not recommended.** On a full 17:38 meeting: of 8 arbitrated chunks, 8 resembled WhisperX and 0 resembled turbo — the arbiter reproduces WhisperX for 195 s of work that WhisperX alone does in 22 s. Where it *does* diverge it degrades: `Mongo-dbcrm-hn` came out as `Mongo guión bajo DB CRM guión bajo HN`, unusable in a query. The two legs are not peers either — turbo emitted 3367 words with 50 punctuation marks against WhisperX's 1323 with 341, i.e. it hallucinates unpunctuated repetition. The arbiter never hears the audio; it can only judge textual plausibility, and a technical identifier is exactly what looks *im*plausible in Spanish. Prefer `TRANSCRIBER=whisperx`.
     Note for anyone writing a script that calls `transcribe_ensemble` directly: the `spawn` context means the script **must** guard with `if __name__ == "__main__":`, or the child re-imports the main module, WhisperX dies, and the ensemble degrades silently to whisper.cpp alone.
